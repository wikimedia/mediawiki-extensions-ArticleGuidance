...
When a user types in the search box, `useWikidataSearch` debounces the input and fires two requests in parallel:
...
Once both complete, the outlines' article-type Q-IDs are grouped by their entry's `matchVia`. Two more requests then fire in parallel:

1. **`fetchEntityClaims`** — a single `wbgetentities` call that fetches the relevant claims (P31, P106, P171, P18, sitelinks, etc.) and the labels and descriptions (in the user's language) for all 20 candidate Q-IDs at once.
